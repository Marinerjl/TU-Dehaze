...
class energy_dc_loss(nn.Module):
    ...
    def forward(self, img, y_pred):
        ...
        norm_dc = self.get_dark_channel(normalized_image, self.w)
        ...
        t_slide = guided_filter(guidance, t_slide)

        tsl_flat = t_slide.view(-1)
        epsilon = 5
        n_elements = int(epsilon * tsl_flat.numel() / 100)
        _, indices = torch.topk(tsl_flat, n_elements, largest=False)
        bottom_values = tsl_flat[indices]
        tmin = torch.mean(bottom_values)
        if tmin < 0:
            tmin = 0.1

        t_slide = torch.max(t_slide, torch.tensor(tmin)).to(t_slide.device)
        t_slide = torch.cat([t_slide] * 3, 1)
        ...
        loss = F.smooth_l1_loss(y_pred, t_slide)
...
class energy_dc_bc_loss(nn.Module):
    ...
    def __init__(self, k, w=15, p=0.001, omega=0.95, eps=1e-5, lambda1=2, lambda2=1e-4):
        super(energy_dc_bc_loss, self).__init__()
...
class energy_cap_loss(nn.Module):
    ...
    def forward(self, img):
        ...
        cap_prior = v - s
        loss = mse(cap_prior, torch.zeros_like(cap_prior))
