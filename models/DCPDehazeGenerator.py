...
def get_dark_channel(img, neighborhood_size):
    shape = img.shape
    if len(shape) == 4:
...
def atmospheric_light(img):
    neighborhood_size = 5
    dark_img = get_dark_channel(img, neighborhood_size)
...
def dcp(img):
    ...
    A = atmospheric_light(imgPatch)
    map_A = A.repeat(1, 1, height, width)
    trans_raw = 1 - omega * get_dark_channel(imgPatch / map_A, neighborhood_size)
    T_DCP = guided_filter(guidance, trans_raw)
    T_DCP = T_DCP.expand((img.shape))
    return T_DCP
