...
class DCPDN(nn.Module):
    def __init__(self):
        ...
        #     nn.Conv2d(4, 3, kernel_size=7, padding=0, bias=False),
        #     nn.BatchNorm2d(3),
        #     nn.ReLU(True)
        # )
    ...
    def forward(self, x):
        ...
        transmap = torch.sigmoid(self.refine3(dehaze))
        # transmap = torch.cat([transmap]*3, 1)

        return transmap

if __name__ == "__main__":
    x = torch.rand((1, 3, 256, 256)).cuda("cuda:0")
    net = DCPDN()
    net = net.cuda()
    transmap = net(x)
    print(net)
    print(transmap.shape) # torch.Size([1, 3, 256, 256])
